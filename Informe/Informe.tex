...
\begin{enumerate}
	...
	Pero esto es así, cuando se ha desacoplado el acoplamiento con $i^{r}_{ds}$ en el eje q. Vemos que cuando $I^{r}_{ds0}(t)\neq 0$ se produce una migración de los parámetros de la matriz A que influyen sobre $\omega_{m}$ (velocidad) y $i^{r}_{qs}$, además se producirá también una migración de los puntos de operación $I^{r}_{qs0}(t)$ y $\Omega_{m0}$.
	...
	Se concluye entonces que si no se desacopla la influencia de $i^{r}_{ds}$ en el eje q, una $i^{r}_{ds}\neq 0$, no solo va a producir un cambio sobre el torque, si no que va a cambiar notablemente la dinámica del sistema (matriz A) y también va a influir considerablemente sobre el punto de operación $I^{r}_{qs0}$.\\
