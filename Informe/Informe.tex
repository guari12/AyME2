...
\begin{equation}
	G^{i^{r}_{qs}}_{v^{r}_{qs}}(s)=\frac{i^{r}_{qs}(s)}{V_{qs}(s)}=\frac{J_{eq}.s+b_{eq}}{(J_{eq}L_{q}s^{2}+(R_{s}J_{eq}+L_{q}b_{eq})s + \frac{3}{2}P_{p}^{2}\lambda ^{r'2}_{m}+R_{s}b_{eq}).s}
	\label{eq:2.1.2.ff.6}
	\end{equation}
	\begin{equation}
	G^{i^{r}_{qs}}_{T_{leq}}(s)=\frac{i^{r}_{qs}}{T_{leq}(s)}=\frac{P_{p}\lambda^{r'}_{m}}{(J_{eq}L_{q}s^{2}+(R_{s}J_{eq}+L_{q}b_{eq})s + \frac{3}{2}P_{p}^{2}\lambda ^{r'2}_{m}+R_{s}b_{eq}).s}
	\label{eq:2.1.2.ff.7}
	\end{equation} 
\end{enumerate}
...
A continuación se presentan las respuestas del sistema.
	...
	Otro punto importante a notar es la gran acción de control $v^{r}_{ds}$ que se realiza para mantener $i^{r}_{ds}=0$ durante el transitorio del sistema y con esta aumenta en magnitud cuando se da la perturbación de carga.\\
...
Recordando la ec.\ref{eq:2.1.2.k.7} y la ec. \ref{eq:2.1.2.k.5} se obtiene:
	...
	\begin{equation}
	T^{`}_{m}(s)=\frac{1}{\frac{L_{q}}{R_{q}}s+1}[T^{*'}_{m}(s)+b_{m}\omega_{m}(t)] \ ; \ G_{I}(s)=\frac{L_{q}}{R_{q}}s+1
	\label{eq:2.1.2.q.4}
	\end{equation}
...
Asi obtenemos:
	...
	Si consideramos que el lazo de corriente lo suficientemente rápido ($G_{I}\equiv 1$), entonces los polos del sistema se determinan por el polinomio característico:
	\begin{equation}
...
\begin{itemize}
	...
	\item También sería interesante, considerar un modelo que contemple la variación de las resistencias de los bobinados en función de la temperatura para un ajuste más preciso del sistema.
\end{itemize}
