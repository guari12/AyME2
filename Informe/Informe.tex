...
-\frac{T_{s}^{\circ}\left ( t \right )-T_{amb}^{\circ}\left ( t \right )}{R_{ts-amb}}}{C_{ts}}
		...
		\\
		\\
		Este modelo \textbf{NL} puede ser representado mediante diagramas de bloques en \textbf{Simulink} como se observa en la siguiente figura \ref{fig:sistGlobalNL}:
		\begin{figure}[h!]
			\centering
			\includegraphics[width=0.9\textwidth]{2_1_2_ModeloElectricoEquivalente.png}
			\caption{\label{fig:sistGlobalNL} Diagrama en bloques del modelo eléctrico equivalente en \textbf{Simulink} (con transformación de \textbf{Park})}
		\end{figure}
